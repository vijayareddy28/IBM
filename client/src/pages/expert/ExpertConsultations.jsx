 ...
 * Dynamic — fetches real data from /api/expert/consultations.
 */

import { useState, useEffect, useCallback } from 'react';
import {
  MessageSquare, Calendar, Clock, CheckCircle, Loader2,
  Video, MessageCircle, AlertCircle, RefreshCw, ChevronDown,
} from 'lucide-react';
import { fetchExpertConsultations, updateExpertConsultationStatus } from '../../services/expertService';
...
const ExpertConsultations = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    setUpdating(id);
    try {
      await updateExpertConsultationStatus(id, newStatus);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard icon={MessageSquare} label="Total"     value={total}     color="bg-violet-50 text-violet-600" />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {['Patient / Case', 'Reason', 'Date & Time', 'Mode', 'Status', 'Action'].map((h) => (
                    ...
                    <tr key={c._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5">
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700">
                          {modeIcon(c.consultationType)} {c.consultationType?.replace('_', ' ') || '—'}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        ...
                        {transitions.length > 0 ? (
                          <div className="relative">
                            <select
                              ...
                              value=""
                              className="text-xs border border-gray-200 rounded-lg px-2 py-1 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-violet-400 cursor-pointer disabled:opacity-50"
                            >
