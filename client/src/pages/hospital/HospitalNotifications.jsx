/**
 * HospitalNotifications — CarePath AI
 * In-app notification feed for the hospital account.
 */
...
import {
  fetchHospitalNotifications, markHospitalNotificationRead, markAllHospitalNotificationsRead,
...
const NotificationItem = ({ notif, onRead }) => {
  ...
  return (
    <div className={`flex items-start gap-3 px-4 py-3 border-b border-gray-100 last:border-0 ${notif.read ? 'bg-white' : 'bg-emerald-50/30'}`}>
      ...
      {!notif.read && (
        <button onClick={() => onRead(notif._id)} className="shrink-0 p-1 text-gray-400 hover:text-emerald-600 transition-colors">
...
const HospitalNotifications = () => {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await fetchHospitalNotifications({ unreadOnly: unreadOnly ? 'true' : 'false' });
  ...
  const handleRead = async (id) => {
    try {
      await markHospitalNotificationRead(id);
  ...
  const handleReadAll = async () => {
    try {
      await markAllHospitalNotificationsRead();
        ...
        <div className="flex gap-2">
          ...
          {unreadCount > 0 && (
            <button onClick={handleReadAll}
              className="flex items-center gap-1.5 text-xs font-medium text-emerald-600 border border-emerald-200 rounded-lg px-3 py-1.5 hover:bg-emerald-50 transition-colors">
        ...
        {[false, true].map((v) => (
          <button key={String(v)} onClick={() => setUnreadOnly(v)}
            className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${
              unreadOnly === v ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'
            }`}>
            {v ? 'Unread only' : 'All'}
          </button>
        ))}
